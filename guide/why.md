...
关于blq-test是什么，前面已经介绍过了.这里介绍一下blq-test带来的意义,主要解决通用表单，列表，详情的写法，真正做到用数据（field）去驱动试图，下面一个常用的搜索框，感受一下
...
```
### 现在
嗯，你是不是感觉翻了很久的图片，才翻到这里。过时的写法就是这样的，代码冗余，看着心累。现在看看使用blq-test中的WithSearch组件会会发生什么；
